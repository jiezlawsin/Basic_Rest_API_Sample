...
(function($) {
    $.fn.sumodate = function(options) {
        ...
        return this.each(function() {
			...
			html += '<select name="drop_month-'+input.random+'" id="drop_month-'+input.random+'" class="drop_month '+ options.monthClass +'">'+
			...
			html += '<select name="drop_day-'+input.random+'" id="drop_day-'+input.random+'" class="drop_day '+ options.dayClass +'">'+
		...
		html += '<select name="drop_year-'+input.random+'" id="drop_year-'+input.random+'" class="drop_year '+ options.yearClass +'"><option value="" disabled>Year</option>';
		...
		if (inputDate != '') {
			...
			if (options.defaultDate =='current'){
				defaultMonth = pad(d.getMonth() + 1);
				defaultDay = d.getDate();
				defaultYear = d.getFullYear();
			}
			else {
				var dateVal = new Date(options.defaultDate);
				defaultMonth = pad(dateVal.getMonth() + 1);
				defaultDay = dateVal.getDate();
				defaultYear = dateVal.getFullYear();
			}
				$('#drop_month-'+input.random).val(defaultMonth);
				$('#drop_day-'+input.random).val(defaultDay);
				$('#drop_year-'+input.random).val(defaultYear);
		}
		defMonth = $('#drop_month-'+input.random).val();
		defDay = $('#drop_day-'+input.random).val();
		defYear = $('#drop_year-'+input.random).val();
		$('#drop_date-'+input.random).val(defYear+'-'+defMonth+'-'+defDay);
		date.remove();
	});
